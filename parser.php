<?php
define('PX_VER_PARSER', '22117');
define('PX_DATE_PARSER', '2011-01-27');
...
function proxy_GET_FB($url, $vPostGet='GET', $vPostData='') {
	...
	#    if (strpos($vHTTPResponse, '302 Found') !== false){
	#        $Cnt302++;
	#        AddLog2("proxy_GET_FB: 302 Found, not logged in?");
	#        return 0;
	#    }
...
function Arbeit() {
		...
		if(is_array($vGiftReqs)) {
			if(count($vGiftReqs)>0) {
				...
				foreach($vGiftReqs as $vI => $vData) {
					...
					#file_put_contents('giftreq'.__LINE__.'.html', $vResponse."\n\n####\n\n"."http://www.facebook.com/ajax/reqs.php?__a=1"."\n\n####\n\n".$vData['post_data']);
					...
					if ($enable_acceptgifts_sendback) {
						...
						#file_put_contents('giftreq'.__LINE__.'.html', print_r($vTYForms,true));
						...
						foreach($vTYForms[0] as $vJ => $vTYForm) {
							if(stripos($vTYForm, 'thank you') !== false || stripos($vTYForm, 'send to') !== false) {
								...
								#file_put_contents('giftreq'.__LINE__.'.html', $vJ.' -> '.print_r($vTYForm,true));
								preg_match_all('/.*action="([^"]*)".*/ims', $vTYForm, $vAction);

								preg_match_all('/.*giftRecipient=([^&]*).*type="([^"]*)".*content="([^"]*)".*id="([^"]*)".*post_form_id=([^&]*).*/ims', $vTYForm, $vTYFields);

								#file_put_contents('giftreq'.__LINE__.'.html', $vJ.' -> '.print_r($vTYFields,true));
								...
								#file_put_contents('giftreq'.__LINE__.'.html', $vResponse2."\n\n####\n\n"."http://www.facebook.com/fbml/ajax/prompt_send.php?__a=1"."\n\n####\n\n".$vPostData);
								...
								#file_put_contents('giftreq'.__LINE__.'.html', $vResponse3."\n\n####\n\n"."http://www.facebook.com/fbml/ajax/prompt_send.php?__a=1"."\n\n####\n\n".$vPostData);

								$vResponse4 = proxy_GET_FB(html_entity_decode($vAction[1][0]), 'POST', '');

								#file_put_contents('giftreq'.__LINE__.'.html', $vResponse4."\n\n####\n\n".html_entity_decode($vAction[1][0])."\n\n####\n\n".'');
	...
	if ($enable_harvest_building == 1) {
		...
		if ($enable_harvest_building_horse == 1) {
			...
			if ($enable_harvest_building_wworkshop == 1 || $enable_harvest_building_snowman == 1) {
...
function CheckAMF2Rewards($amf2) {
	#    file_put_contents('rew_data_raw_tmp_'.time().'.txt', print_r($amf2->_bodys[0],true), FILE_APPEND);
	...
	foreach($amf2->_bodys[0]->_value['data'] as $vReward) {
		...
		if(!$vFound) {
			#            save_botarray ($amf2, 'amf2_'.date('Ymd_his_u').'.txt');
...
function Parser_ReadReq() {
	...
	#file_put_contents('giftreq'.__LINE__.'.html', print_r($vHTML,true));
	...
	#file_put_contents('giftreq'.__LINE__.'.html', print_r($vForms,true));

	$vGiftRequests = array();
	foreach($vForms[0] as $vI => $vForm) {
		preg_match_all('/name="([^"]*)" value="([^"]*)"/ims', $vForm, $vNameValues);

		#file_put_contents('giftreq'.__LINE__.'.html', print_r($vNameValues,true));

		preg_match_all('/<input[^>]*value="([^\"]*)"[^>]*name="actions\[([^>]*)][^>]*>?/ims', $vForm, $vActions);

		#file_put_contents('giftreq'.__LINE__.'.html', print_r($vActions,true));

		preg_match_all('/<a href="http:\/\/apps\.facebook\.com\/.*?>(.*?)<\/a>/ims', $vForm, $vAppNameValues);

		#file_put_contents('giftreq'.__LINE__.'.html', print_r($vAppNameValues,true));

		preg_match_all('/name="fb_dtsg" value="([^"]*)"/ims', $vForm, $vDTSGValues);

		#file_put_contents('giftreq'.__LINE__.'.html', print_r($vDTSGValues,true));
